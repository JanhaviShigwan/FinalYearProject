...
exports.sendTestAnnouncementEmail = async (req, res) => {
  try {
    const settings = await getOrCreateSettings();
    const announcementSettings = settings.announcementSettings || {};

    if (!announcementSettings.enableAnnouncementEmails) {
      return res.status(400).json({
        message: "Announcement emails are disabled in settings",
      });
    }

    const subject = announcementSettings.defaultEmailSubject || "EventSphere Announcement";
    const signature = announcementSettings.defaultEmailSignature || "EventSphere Admin Team";

    const html = `
      <div style="font-family: Arial, sans-serif; padding: 24px; background: #F6F1EB; color: #3F3D56;">
        <div style="max-width: 640px; margin: auto; background: #FFFFFF; border-radius: 12px; padding: 24px; border: 1px solid #EED8D6;">
          <h2 style="margin: 0 0 12px 0; color: #9B96E5;">Announcement Test Email</h2>
          <p style="margin: 0 0 12px 0;">This is a test email from your Admin Settings configuration.</p>
          <p style="margin: 0 0 16px 0;"><strong>Subject Prefix:</strong> ${subject}</p>
          <div style="padding: 12px 14px; background: #F9F5FF; border-left: 4px solid #9B96E5; border-radius: 8px;">
            ${signature}
          </div>
        </div>
      </div>
